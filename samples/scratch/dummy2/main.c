...
#include "xpm/breakpoint_on.xpm"
#include "xpm/breakpoint_off.xpm"   """""
    #include "xpm/breakpoint_invalid.xpm"
#include "xpm/debugcursor.xpm"
#include "FileEditorGroupWnd.h"
   


####include "MainFrame.h"
#include "ErrorListWnd.h"
#include "FindSymbolDlg.h"
#include "apcpuas/as_emitter.h"
#include "FileEditorStyles.h"
#include "document/SimulatorRoot.h"
#include "document/DebuggerSession.h"
#include "document/Workspace.h"
#include "buildutil/BuildUtil.h"
#include "lexer/LexClang.h"
...
BEGIN_EVENT_TABLE(FileEditorWnd,     FileEditorWnd_Auto    asdf    asdfa)
...
#define AUTOCOMP_IMG_OTHER 0
#define AUTOCOMP_IMG_F 1
#define AUTOCOMP_IMG_V 2
#define AUTOCOMP_IMG_M 3
#define  AUTOCOMP_IMG_T 4
#define AUTOCOMP_IMG_E 5
...
//
// Maximum number of items to show in the autocomplete box
// If the real number of results is over this limit, only the entries that start with the current word will
// be included
#define MAX_AUTOCOMPLETE_ITEMS 100
...
FileEditorWnd::FileEditorWnd(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
	...
	, m_timer(this)
	, m_vimmode(m_textCtrl, *this)
{
	m_textCtrl->SetSizeHints(wxSize(640, 350));

	m_textCtrl->RegisterImage(AUTOCOMP_IMG_OTHER, gImageList16x16.GetBitmap(IMG_INDEX_16x16_CODE_OTHER));
	m_textCtrl->RegisterImage(AUTOCOMP_IMG_F, gImageList16x16.GetBitmap(IMG_INDEX_16x16_CODE_F));
	m_textCtrl->RegisterImage(AUTOCOMP_IMG_V, gImageList16x16.GetBitmap(IMG_INDEX_16x16_CODE_V));
	m_textCtrl->RegisterImage(AUTOCOMP_IMG_M, gImageList16x16.GetBitmap(IMG_INDEX_16x16_CODE_M));
	m_textCtrl->RegisterImage(AUTOCOMP_IMG_T, gImageList16x16.GetBitmap(IMG_INDEX_16x16_CODE_T));
	m_textCtrl->RegisterImage(AUTOCOMP_IMG_E, gImageList16x16.GetBitmap(IMG_INDEX_16x16_CODE_E));

	document::addCallback(this, CALLBACK_BREAKPOINTS_CHANGED | CALLBACK_DBGBRK | CALLBACK_STATE_CHANGED,
							  [=]()
	{
	...
	m_textCtrl->Connect( wxEVT_CHAR, wxKeyEventHandler(FileEditorWnd::OnChar),
						NULL, this);
	//m_textCtrl->Disconnect(wxEVT_CHAR, wxKeyEventHandler(FileEditorWnd::OnChar), NULL, this);
	m_textCtrl->Connect(wxEVT_CHAR_HOOK, wxKeyEventHandler(FileEditorWnd::onCharHook), NULL, this);
...
FileEditorWnd::~FileEditorWnd()
{
	...
	{
		m_file->dirty = false;
		m_file->saveFunc = nullptr;
	}
	document::removeCallback(this);
}

void FileEditorWnd::setVimMode(bool state)
{
	if (state)
	{
		if (!m_vimmode.isEnabled())
			m_vimmode.enable();
	}
	else
	{
...
document::FilePtr FileEditorWnd::getFile()
{
	return m_file;
}
...
void FileEditorWnd::setCommonStyle()
{
	...
	m_textCtrl->SetSelBackground(true, clSelectionBkg);
	m_textCtrl->SetAdditionalSelBackground(clSelectionBkg);
}

void FileEditorWnd::updateReadOnly()
{
	using namespace cz::apcpuide::document;
	m_textCtrl->SetReadOnly( m_file->isAutoGenerated() || gSession->getState()!=kSessionState_None);
}

void FileEditorWnd::setFile(document::FilePtr file, int line /*=0*/, int col /*=0*/, bool columnIsOffset,
							bool addToCursorHistory)
{
	using namespace cz::apcpuide::document;
	...
	if (file != m_file)
	{
		...
		cz::UTF8String ext = m_file->getExtension();
		...
		if (lang)
		{
			if (lang->lexer == SCLEX_CPP)
			{
				m_isCFile = true;
				m_textCtrl->SetLexer(cz::cparser::getLexerID());
				m_textCtrl->SetProperty("lexer.clang.filename", wxString(m_file->getFullPath().c_str()));
			}
			else
			{
		...
		//m_textCtrl->SetText(contents);
		m_textCtrl->SetReadOnly(false);
		m_textCtrl->LoadFile(file->getFullPath().c_str(), wxTEXT_TYPE_ANY);
		m_file->filetime = cz::Filetime::get(m_file->getFullPath(), Filetime::kModified);
		...
		{
			document::File* thisfile = m_file.get();
			m_file->saveFunc = [thisfile, this]()
			{
				this->save();
			};
		}
		...
		if (m_isCFile)
		{
			// This is to disable constant colourising while editing.
			// I'm colourising the entire document explicitly when opening, then disable the lexer until further notice,
			// so I can trigger full colourising after a few idle seconds
		...
		// This seems to be needed, otherwise when opening a file and doing a goto line/column at the same time,
		// the horizontal scroll will be messed up, apparently due to the custom margins.
		// Somehow, what the scrollbar shows and and internal scrolled value will be out of sync
		// Found this fix mostly with trial and error.
		m_textCtrl->Fit();
		this->Layout();
	}
	...
	m_textCtrl->EnsureCaretVisible();
	m_textCtrl->VerticalCentreCaret();
	if (addToCursorHistory)
	{
		auto pos = getCursorLocation();
		gFileEditorGroupWnd->cursorHistory_AddHistory(document::FileLocation(m_file, pos.first, pos.second));
	}
}
...
{
	cz::Filetime t = cz::Filetime::get(m_file->getFullPath(), Filetime::kModified);
	if (t<= m_file->filetime)
		return;
	...
	setFile(f, -1, 0);
	//m_textCtrl->SetReadOnly(false);
	//m_textCtrl->LoadFile(m_file->getFullPath().c_str());
	//updateReadOnly();
}
...
void FileEditorWnd::onMarginClick(wxStyledTextEvent& event)
{
	...
	switch(event.GetMargin())
	{
		case MARGIN_BREAKPOINTS:
		{
			...
			gSession->addOrDeleteBreakpoint(m_file, lineclick);
		}
		break;
		case MARGIN_FOLD:
		{
...
void FileEditorWnd::OnTextChanged(wxStyledTextEvent& event)
{
		...
		(flags&wxSTC_MOD_DELETETEXT) == wxSTC_MOD_DELETETEXT)
	{
		...
		if (m_isCFile)
		{
			// Restart the colourising timer every time we change the document
			m_timer.Start(2000, true);
		}

	}

	if (flags&wxSTC_MOD_CONTAINER)
	{
		if (flags&wxSTC_PERFORMED_UNDO)
		{
			m_vimmode.onCustomUndo(event.GetToken());
		}
	}

	if (event.GetLinesAdded())
	{
		int currentLine = m_textCtrl->LineFromPosition(event.GetPosition()) + 1;
	}
...
std::pair<cparser::CallTip, std::vector<cparser::AutoCompleteResult>> FileEditorWnd::getAutoCompleteResults(std::string& token)
{
	int line = 0, col = 0;
	getPositionForParser(line, col);
	wxString l = m_textCtrl->GetLine(line - 1);
	int startCol = findCurrentWordStart(m_textCtrl->GetCurrentPos(), token);
	cparser::CallTip calltip;
	auto results =
		document::gWorkspace->getCParser().codeCompletion(m_file->getFullPath().c_str(), line, startCol, token, calltip);

	return {std::move(calltip), std::move(results)};
}
...
void FileEditorWnd::showAutoComplete()
{
	...
	m_textCtrl->AutoCompSetIgnoreCase(true);
	wxString list;
	int test = 0;

	std::string token;
	auto results = getAutoCompleteResults(token);
	bool exactPrefix = false;
	if (results.second.size() > MAX_AUTOCOMPLETE_ITEMS)
		exactPrefix = true;
	int count = 0;
	for (auto& r : results.second)
	{
		if (exactPrefix && cz::ci_find_substr(r.text, token)!=0)
			continue;
		...
		int img;
		if (r.kind == ItemKind::Function)
			img = AUTOCOMP_IMG_F;
		else if (r.kind == ItemKind::Field || r.kind == ItemKind::Variable)
			img = AUTOCOMP_IMG_V;
		else if (r.kind == ItemKind::Macro)
			img = AUTOCOMP_IMG_M;
		else if (r.kind == ItemKind::Struct || r.kind==ItemKind::Union || r.kind == ItemKind::Typedef || r.kind == ItemKind::Enum)
			img = AUTOCOMP_IMG_T;
		else if (r.kind == ItemKind::EnumVal)
			img = AUTOCOMP_IMG_E;
		else
			img = AUTOCOMP_IMG_OTHER;

		count++;
		list += cz::formatString("%s?%d", r.text.c_str(), img);
	}

	if (count)
	{
		m_textCtrl->AutoCompShow(token.size(), list);
		//m_textCtrl->AutoCompSelect(token);
	}
}
...
{
	std::string token;
	auto res = getAutoCompleteResults(token);
	if (res.first.tip == "" || !res.first.symbolInfo.isValid())
		return;

	if (m_calltipPos==-1 || updatePos)
		m_calltipPos = m_textCtrl->GetCurrentPos();

	std::string full = res.first.symbolInfo.comment + "\n" + res.first.tip;
	m_textCtrl->CallTipShow(m_calltipPos, full);
	m_textCtrl->CallTipSetHighlight(res.first.symbolInfo.comment.size() + res.first.hltStart + 1,
									res.first.symbolInfo.comment.size() + res.first.hltEnd + 1);
}
...
void FileEditorWnd::OnCharacterAdded(wxStyledTextEvent& event)
{
	...
	if (chr== '\n' || chr=='\r')
	{
		...
		if (lineInd==0)
			return;
		int currIndent = m_textCtrl->GetLineIndentation(currentLine);

		m_textCtrl->SetLineIndentation(currentLine, lineInd);
		// Check if the line is all whitespaces/empty
		vim::VimPos pos(m_textCtrl); // #TODO: Remove this? Should not be using any vim mode stuff here
		if (pos.isBlankLine())
			pos.lineEnd().apply();
		else
			pos.lineFirstNonBlank().apply();
		return;
	}
...
void FileEditorWnd::save()
{
	...
	try {
		m_textCtrl->SaveFile(m_file->getFullPath().widen());
		m_file->dirty = false;
		m_file->stamp++;
		m_file->filetime = Filetime::get(m_file->getFullPath(), Filetime::kModified);
		if (m_file->generatedFile)
			m_file->generatedFile->annotation = "(outdated)";
		fireCustomAppEvent(CAE_FileSaved, (uint64_t)this);
	}
...
void FileEditorWnd::OnChar(wxKeyEvent& event)
{
	if (m_vimmode.isEnabled())
	{
		if (!m_vimmode.processOnChar(event))
			event.Skip();
	}
}
...
void FileEditorWnd::onCharHook(wxKeyEvent& event)
{
	...
	else if (modifiers == wxMOD_CONTROL && keycode == WXK_F7) // Compile current file
	{
		try
		{
			buildutil::compileFile(document::BuildVariables(), m_file, document::gWorkspace->getActiveConfigurationName());
	...
	else
	{
		if (m_vimmode.isEnabled())
		{
			if (!m_vimmode.processOnCharHook(event))
				event.Skip();
		}
		else
		{
...
{
	m_textCtrl->SetIndentationGuides( gMainFrame->viewIndentationGuides() ? wxSTC_IV_LOOKBOTH : wxSTC_IV_NONE);
	m_textCtrl->SetViewWhiteSpace(gMainFrame->viewWhiteSpaces() ? wxSTC_WS_VISIBLEALWAYS : wxSTC_WS_INVISIBLE);
	m_textCtrl->SetViewEOL(gMainFrame->viewEOL() ? true : false);
...
void FileEditorWnd::onCustomAppEvent(const CustomAppEvent& evt)
{
	switch (evt.id)
	{
		case CAE_BuildFinished:
			updateErrorMarkers();
			break;
		case CAE_ViewIndentation:
		case CAE_ViewWhitespace:
		case CAE_ViewEOL:
			updateViewOptions();
			break;
		default:
...
std::pair<const char*, int> FileEditorWnd::getDocumentData()
{
	//wxCharBuffer buf = m_textCtrl->GetTextRaw();
	auto buf = m_textCtrl->GetCharacterPointer();
	auto len = m_textCtrl->GetLength();
...
//
// VimModeListener Interface
//
void FileEditorWnd::vimLog(const char* txt)
{
	CZ_LOG(logDefault, Log, txt);
    }  

void FileEditorWnd::vimUpdateStatus(const   char* txt)
{
	gFileEditorGroupWnd->getVimStatusBar()->SetLabelText(txt);
}
